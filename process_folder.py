...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("Script to run CountGD++ on a folder of images")
    p.add_argument("--input_folder", type=str, help="folder containing images for evaluation")
    p.add_argument("--prompts", type=str, help="path to JSON file containing object prompts")
    p.add_argument("--output_folder", type=str, help="folder where to output predicted boxes and counts")
    p.add_argument("--pretrain_model_path", type=str, default="checkpoints/countgd_plusplus.pth", help="model checkpoint")
    p.add_argument("--conf_thresh", type=float, default=0.23, help="confidence threshold. Increase to reduce false positives. Decrease to get more detections.")
    p.add_argument("--image_enhance", action='store_true', help="apply AI-based superresolution to images before counting for potential performance boost")
    p.add_argument("--vis_output", action='store_true', help="save images of the output boxes overlain onto the input images")
...
def get_boxes_from_prediction(model_output, text, keywords = "", conf_thresh=0.23):
...
def predict(model, transform, image, positive_text, positive_prompts, negative_texts, negative_prompts, device, conf_thresh):
    ...
    return get_boxes_from_prediction(model_output, positive_text, keywords, conf_thresh)

def _predict(image, positive_text, positive_prompts, negative_text, negative_prompts, conf_thresh):
        return predict(model, transform, image, positive_text, positive_prompts, negative_text, negative_prompts, device, conf_thresh)

def count(image, positive_text, positive_prompts, negative_texts, negative_prompts, conf_thresh, vis_output):
        if positive_prompts is None:
            positive_prompts = {"image": image, "points": []}
        boxes, scores = _predict(image, positive_text, positive_prompts, negative_texts, negative_prompts, conf_thresh)
        ...
        if vis_output:
          output_img = generate_heatmap(image, boxes)
            ...
            output_img = None
        (x0, y0, box_w, box_h) = get_box_coords_from_boxes(image, boxes)
        boxes = np.stack((x0, y0, x0 + box_w, y0 + box_h), axis=-1)

        return boxes, scores, output_img
...
images = list_images_in_split(args.input_folder)
num_images = len(images)

# Get object prompts
with open(args.prompts, 'r') as prompt_file:
    prompts = json.load(prompt_file)

positive_prompts = {}
positive_prompts["text"] = prompts["positive"]["text"]
positive_exemplar_image_name = prompts["positive"]["exemplars"]["image"]
positive_exemplar_image = Image.open(positive_exemplar_image_name).convert("RGB")
positive_prompts["image"] = positive_exemplar_image
positive_prompts["points"] = []
positive_exemplar_boxes = prompts["positive"]["exemplars"]["boxes"]
for x1, y1, x2, y2 in positive_exemplar_boxes:
    positive_prompts["points"].append([x1, y1, 2.0, x2, y2, 3.0]) # gradio image prompter format for box

negative_prompts_list = []
for negative_prompts_i in prompts["negative"]:
    negative_prompts = {}
    negative_prompts["text"] = negative_prompts_i["text"]
    negative_exemplar_image_name = negative_prompts_i["exemplars"]["image"]
    negative_exemplar_image = Image.open(negative_exemplar_image_name).convert("RGB")
    negative_prompts["image"] = negative_exemplar_image
    negative_prompts["points"] = []
    negative_exemplar_boxes = negative_prompts_i["exemplars"]["boxes"]
    for x1, y1, x2, y2 in negative_exemplar_boxes:
        negative_prompts["points"].append([x1, y1, 2.0, x2, y2, 3.0]) # gradio image prompter format for box
    negative_prompts_list.append(negative_prompts)

# Create output directory and dictionary to save results
os.makedirs(args.output_folder, exist_ok=True)
results = {}

# Process images
for file_ind in range(len(images)):
    logging.info("Processing image %d/%d", file_ind + 1, num_images)
    file_path = images[file_ind]
    ...
    boxes_pred, scores_pred, output_image = count(image, positive_prompts["text"], positive_prompts, [negative_prompts["text"] for negative_prompts in negative_prompts_list], negative_prompts_list, args.conf_thresh, args.vis_output)
    
    for ind in range(boxes_pred.shape[0]):
        bbox = boxes_pred[ind,:].tolist() 
        score = float(scores_pred[ind])

        results[file_name]["count"]+=1
        results[file_name]["boxes"].append(bbox)
        results[file_name]["scores"].append(score)

    if args.vis_output:
        output_image.convert("RGB").save(os.path.join(args.output_folder, file_name))
        


# Write results to json.
out_json_file_name = os.path.join(args.output_folder, "results.json")
with open(out_json_file_name, "w") as f:
    json.dump(results, f)
print(f"Saved results to {out_json_file_name}")

print("\n==== Summary ====")
print(f"{args.input_folder:30s} | images: {num_images:5d}")